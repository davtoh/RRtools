...
def getData(path=__file__):  # FIXME not working windows syntax under linux
    ...
    if os.path.isdir(path):  # is path to folder
        ...
        # base,filename = os.path.dirname(dirname), os.path.basename(dirname)
        base, filename = os.path.split(dirname)
        drive, dirname = os.path.splitdrive(
            dirname[:len(dirname) - len(filename)])
        return [drive, dirname, filename, ext]
...
def changedir(filepath, dirname, ext=True):
    ...
    """
    if ext:
        data = getData(filepath)[2:]
    else:
        data = getData(filepath)[2:3]
...
def mkPath(path):  # FIXME: not sure if this is correct
    ...
    try:
        # if not os.path.exists(path): os.makedirs(path) # not
        # os.path.isdir(path)
        os.makedirs(path)
        ...
        if exception.errno == errno.EEXIST:
            if os.path.isfile(path):
                raise Exception(
                    "Folder not created. There is a file with the same name at {}".format(path))
...
def rmFile(filepath):
    ...
    """
    try:
        # enclose in try/except, who knows files could be deleted
        os.remove(filepath)
    except OSError:
...
def getSep(path, pattern='/\\'):
    ...
    """
    i = len(path)
    while i and path[i - 1] not in pattern:
        i -= 1
...
def quickOps(path, comp):
    ...
    for i, cp in enumerate(toCompare):
        if cp not in test:  # is a level
            ...
            # TODO: instead of using index i from 'toCompare', find a way to do
            # it directly in 'toCorrect'
            if tfrom and tto:
                if isin:
...
def decoratePath(relative, sep=os.path.sep):
    ...
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            path = f(*args, **kwargs)
            # relative is corrected internally
            return getShortenedPath(correctSep(path, sep), relative)
            # return sep.join(getShortenedPath(path,
            # relative).split(getSep(path)))
        return wrapper
    return decorator
...
def joinPath(absolute, relative):
    ...
    if relative.startswith("\\") or relative.startswith("/"):
        relative = relative[1:]
    # str ensures updated version is processed
...
class Directory(str):
    ...
    def filterdata(data, ispath=None, kwargs=None):
        ...
        if isinstance(data, Directory):
            ...
            if isinstance(data, list):
                ...
                data = {"repr": [data]}
        if kwargs:
            data.update(kwargs)  # update data with kwargs
        if ispath is not None:
            data["ispath"] = ispath
        if "ispath" not in data:
            data["ispath"] = True  # ispath default
        if "repr" in data:
            data["repr"] = Directory.repr2list(
                data["repr"])  # ensure repr is maintained
        else:
    ...
    def repr2list(data, level=0):
        ...
        if isinstance(data, list):  # list defines levels of directories
            if level == 0:  # [level 0, ..., [level 1, [...[level N]...]], level 0]
                # process several objects in the list
        ...
        if isinstance(data, list):
            if len(data) > 1:
                ...
                for i in data[1:]:
                    if ispath:
                        string = joinPath(
                            string, Directory.repr2str(i))  # join paths
                    else:
    ...
    # TODO: implement 2 versions: one with directory-like functionality and
    # other basic for str support
    """
    ### MAGIC FUNCTIONS
