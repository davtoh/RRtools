...
def fastplt(image, cmap=None, title="visualazor", win=None, block=False, daemon=False):
    ...
    # image = plt2bgr(image) # once miniprogram is made this should work ->
    # UPDATED 07/04/16
    wins[-1] += 1
    if FLAG_DEBUG:
        print("fastplt received image type: ", type(image))
    ...
    def myplot():
        ...
        # wins[0]+=1
        if win:
            f.canvas.set_window_title(win)
        else:
            f.canvas.set_window_title("Figure {}".format(wins[-1]))
        if FLAG_DEBUG:
            print("showing now...")
        # plt.ion()
        plt.show()
...
def graph_filter(filters, levels=None, titles=None, win=None,
        ...
        if isinstance(name, basestring):
            try:
                ...
                if "*" in name:
                    parts = name.split("*")
                    f = ["{key}: {{{key}}}".format(
                        key=key) for key in list(params.keys())]
    ...
    if isinstance(filters, FilterBase):
        if win is None:
            win = (getFilterName(filters) + " filter").title()
        filters = [filters]
    elif titles is not None and len(filters) != len(titles) and not isinstance(titles, basestring):
        raise Exception("Titles len {} does not match with filters len {}".format(
            len(titles), len(filters)))

    if win is None:
        win = "Filters Response"
    ...
    for i, f in enumerate(filters):
        ...
        if isinstance(f, FilterBase):
            ...
            # TODO implement getattr() instead with a list of parameters from
            # annotate
            params = {}
            ...
            if annotate:
                ...
                for j, (key, val) in enumerate(items):
                    ...
                    if isinstance(desc, bool):
                        # and len(items)>1: #xp < (xmax+abs(xmin))/2: # label
                        # position at x
                        if desc:
...
def plotPointsContour(pts, ax=None, lcor="k", pcor=None,
    ...
    # get axes, creates and show figure if interactive is ON, disable with
    # plt.ioff()
    ax = ax or plt.gca()
    if pcor is not None:
        ax.plot(pts[:, 0], pts[:, 1], markerfacecolor=pcor,
                marker='o')  # plot points
        ax.plot(pts[:, 0], pts[:, 1], color=lcor)  # and lines
        ax.plot([pts[-1, 0], pts[0, 0]],
                [pts[-1, 1], pts[0, 1]], 'k')  # add last line
    if deg is None:
        deg_list = [""] * len(pts)
    else:
        op = u""
        if deg is True:
            op = u"°"
        deg_list = [u"{:1.1f}{}".format(i, op)
                    for i in vertexesAngles(pts, deg=deg)]
...
def convert2bgr(src, bgrcolor=None):
    ...
    if not np.max(src) > 1:
        ...
        im = src.astype("uint8")
    # ERROR: Source image must have 1, 3 or 4 channels in function
    # cvConvertImage
...
class Plotim(object):
    # FIXME: this code now is buggy, nothing to do. consider replacing it with
    # wxpython or pyqt
    '''
    ...
    def __init__(self, win, im=np.array([[1]]), bgrcolor=(250, 243, 238)):
        ...
        self.controlText = [
            ["zoom(x{self.rxZoom}({self.rx1}-{self.rx2}), y{self.ryZoom}({self.ry1}-{self.ry2})) "]]
        ...
        # key to press to activate builtincontrol
        self.controlkey = cv2.EVENT_FLAG_CTRLKEY
        ...
        # ensures that thickness is > 1
        self.thickness = int(self.fontScale) + 1
    ...
    def rx1(self, value):
        if self.limitrender:
            self._rx2 = limitaxis(self.rx2, min(
                self.rWmax + value, self.maxX), value)
    ...
    def ry1(self, value):
        if self.limitrender:
            self._ry2 = limitaxis(self.ry2, min(
                self.rHmax + value, self.maxY), value)
    ...
    def rx2(self, value):
        if self.limitrender:
            self._rx1 = limitaxis(self.rx1, value, max(
                value - self.rWmax, self.minX))
    ...
    def ry2(self, value):
        if self.limitrender:
            self._ry1 = limitaxis(self.ry1, value, max(
                value - self.rHmax, self.minY))
    ...
    def show(self, frames=None, block=True, daemon=False, clean=True):
        ...
        def _show(frames=None):
            ...
            while not self.close:  # checkLoaded close after processing
                self.pressedkey = cv2.waitKey(
                    self.delayplot)  # & 0xFF  # wait for key
                if self.closefunc():
                    break  # close by closing function
        ...
        if block:
            _show(frames)
        # if called from shell or directly #FIXED __name__ == "__main__" does
        # not work when pickled
        elif isinstance(self, Plotim):
            if FLAG_DEBUG:
                print("multiprocessing...")
            # FIXME i shoud call a miniprogram
            p = Process(target=_show, args=(frames,))
            p.daemon = daemon
            p.start()
        else:  # Workaround to solve problem multiprocessing with matplotlib this sends to shell
            from .serverServices import generateServer, sendPickle
            s, addr = generateServer()
            props = ["python '{script}'".format(
    ...
    def cmdfunc(self, execute=False):
        ...
        def evalcommand(self, cmd, showresult=True, wait=3):
            if type(cmd) is list:  # this configuration lets the user use custom variables
                # FIXME, this is dangerous, it is better to not let any
                # execution to eval
                for command in cmd:
                    try:
                        if "=" in command or "(" in command or "." in command:
                            result = locals()
                            # ,globals() do not use it
                            exec("_ = {}".format(command), result)
                            if showresult:
                                self.plotintime(
                                    items=[[command + " executed"]], wait=wait)
                                if result["_"]:
                                    self.plotintime(
                                        items=[[result["_"]]], wait=wait)
                        else:
                            ...
                            if isinstance(obj, bool):
                                ...
                                if showresult:
                                    if getattr(self, command):
                                        self.plotintime(
                                            items=[[command + " is ON"]], wait=wait)
                                    else:
                                        self.plotintime(
                                            items=[[command + " is OFF"]], wait=wait)
                            elif callable(obj):
                                result = obj()
                                if showresult:
                                    self.plotintime(
                                        items=[["{}".format(result)]], wait=wait)
                    except:
                        exc_type, exc_value, exc_traceback = sys.exc_info()
                        lines = traceback.format_exception(
                            exc_type, exc_value, exc_traceback)
                        msg = lines[-1]  # Log it or whatever here
                        self.plotintime(items=[
                                        ["Error executing " + command + ": " + msg]], wait=wait, bgrcolor=self.errorbackground)
                        print(msg)
                        return False
                if showresult:
                    self.plotintime(
                        items=[[self.cmd + " executed"]], wait=wait)
                return True
            else:
        ...
        if execute and self.cmd != "":  # in execution
            ...
            if command is None:
                if self.cmdformatter:
                    command = self.formatcmd(self.cmd)
                else:
                    command = self.cmd
            if evalcommand(self, command):
                ...
                if command2 is not None:
                    ...
                    self.cmderror = False
                    if not self.cmdcache.count(self.cmd):
                        self.cmdcache.append(command)
                    self.cmd = ""
                else:
                    self.cmderror = False
            else:
        ...
        def format(cmd, strlist, lmissing):
            if len(strlist) != 1:  # ensures toggle functionality for single expression in evalcommand
                for i in range(len(strlist)):
                    cmd = cmd.replace(
                        strlist[i], correct(strlist[i], lmissing))
            return cmd
        ...
        if x is not None and y is not None:
            ...
            if x > self.rW / 2:
                ...
                if y > self.rH / 2:
                    quartile = 1
                    if pixel is not None:
                        text[0].insert(0, pixel)
                else:
                    quartile = 4
                    if pixel is not None:
                        text[0].insert(0, pixel)
            foretext = self.makeoverlay(text, xpad, ypad, bgrcolor, alpha)[0]
            tx, ty = convertXY(x, y, img.shape, foretext.shape,
                               flag, quartile=quartile)  # translation
        else:
            text[0].append(pixel)
            foretext = self.makeoverlay(text, xpad, ypad, bgrcolor, alpha)[0]
            tx, ty = convertXY(
                0, 0, img.shape, foretext.shape, 4)  # translation
        self.showfunc(overlayXY(tx, ty, back=img,
                                fore=foretext))  # show window
        ...
        if items is None:
            if img is not None and self._iswaiting:
                if self._wait > time.time() - self._oldt:
                    ...
                    self.plotatxy(self._timeitems, img, flag=4,
                                  bgrcolor=self._timebgrcolor)
                else:
                    self._timeitems = []
                    self._iswaiting = False
        else:
    ...
    def updaterenderer(self, img=None, zoom=True):
        ...
        if zoom:
            ...
            self.rimg = cv2.resize(convert2bgr(img.copy(), self.bgrcolor),
                                   (self.rW, self.rH), interpolation=self.interpolation)
        if self.showcontrol:
            foretext = self.makeoverlay(
                self.controlText, bgrcolor=self.textbackground)[0]
            overlayXY(*convertXY(0, 0, self.rimg.shape,
                                 foretext.shape, 1), back=self.rimg, fore=foretext)
            ...
            if control is None or self.flags & control:
                ...
                if self.event == self.zoominbutton:  # Zoom in
                    ...
                    if self.rxZoom != mx or self.ryZoom != my:
                        xdiff = np.min((self.rxZoom, self.ryZoom)
                                       ) / self.zoomstep + 1
                        ydiff = xdiff
                        controlled = True
                ...
                if self.event == self.zoomoutbutton:  # Zoom out
                    if self.rxZoom != self.maxX or self.ryZoom != self.maxY:
                        mx = my = self.minzoom
                        xdiff = np.max((self.rxZoom, self.ryZoom)
                                       ) * self.zoomstep + 1
                        ydiff = xdiff
                        controlled = True
                # Move system #
                ...
                if controlled:  # update render parameters
                    self.rx2 = limitaxis(
                        self.x + xdiff, self.maxX, self.minX + mx)
                    self.rx1 = limitaxis(
                        self.x - xdiff, self.rx2 - mx, self.minX)
                    self.ry2 = limitaxis(
                        self.y + ydiff, self.maxY, self.minY + my)
                    self.ry1 = limitaxis(
                        self.y - ydiff, self.ry2 - my, self.minY)
    ...
    def builtincmd(self):
        ...
        if self.pressedkey is not None and self.pressedkey != 255 and self.pressedkey != 27:  # is normal
            def updatelist():
                if not self.cmdfiltered:
                    if self.cmd == "":
                        ...
                        self.cmdfiltered = [i for i in [
                            x for x in self.cmdlist if x.startswith(self.cmd)]]
            ...
            # append to cmd and filter commands
            elif self.pressedkey not in (13, 0, -1) and self.pressedkey < 255:
                try:
    ...
    def builtinplot(self, pixel=None, useritems=None, flag=1, xpad=0, ypad=0, bgrcolor=None, alpha=None):
        ...
        if items != [[]] or pixel is not None:  # if items to plot or pixel color to show
            ...
            if self.staticcoors:
                if pixel is not None:
                    items[0].insert(0, pixel)
                if self.showcontrol and flag == 1:
                    foretext = self.makeoverlay(
                        self.controlText, xpad, ypad, bgrcolor, alpha)[0]
                    self.plotatxy(items, img, x=foretext.shape[1], flag=flag, xpad=xpad, ypad=ypad, bgrcolor=bgrcolor,
                                  alpha=alpha)
                else:
                    self.plotatxy(items, img, flag=flag, xpad=xpad,
                                  ypad=ypad, bgrcolor=bgrcolor, alpha=alpha)
            else:
    ...
    def makeoverlay(self, items, xpad=0, ypad=0, bgrcolor=None, alpha=None):
        ...
        def writetext(text, bgr=None):
            ...
            textbox, baseLine = cv2.getTextSize(
                text, self.fontFace, self.fontScale, self.thickness)  # text dimensions
            bx = (textbox[0], int(textbox[1] * 2))
            foretext = cv2.resize(bgr, bx)  # text image
            # force int, center text
            textOrg = (foretext.shape[1] - textbox[0]
                       ) // 2, (foretext.shape[0] // 2 + textbox[1] // 2)
            cv2.putText(foretext, text, textOrg, self.fontFace,
                        self.fontScale, self.textcolor, self.thickness)
            return foretext
        ...
        def pixelgraph(pixelval, bgr=None, m=None):
            ...
            if m is None:
                textbox, baseLine = cv2.getTextSize(
                    "ss", self.fontFace, self.fontScale, self.thickness)  # text dimensions
                m = int(textbox[1] * 2)
            n = int(m / 6)
        ...
        def evaluate(imgs, items, bgr=None):
            ...
            if isinstance(items, basestring):  # if string
                ...
                try:
                    # just try to get pixel
                    imgs.append(pixelgraph(items, bgr))
                except:
                    # try to convert to string
                    imgs.append(writetext("{}".format(items), bgr))
    ...
    def save(self, strname=None, ext=".png", name="img"):
        ...
        if FLAG_DEBUG and r:
            print(name, "from Plotim saved as", strname)
        return r
...
class MatchExplorer(Plotim):
    ...
    def __init__(self, win, img1, img2, kp_pairs=(), status=None, H=None, show=True, block=True, daemon=True):
        ...
        if status is None:
            # making sure every pair of keypoints is graphed
            status = np.ones(len(kp_pairs), np.bool_)

        kp_pairs = [(dict2keyPoint(i), dict2keyPoint(j)) for i, j in kp_pairs]
        # pair of coordinates for imgf
        p1 = FLOAT([kpp[0].pt for kpp in kp_pairs])
        # pair of coordinates for imgb
        p2 = FLOAT([kpp[1].pt for kpp in kp_pairs]) + (w1, 0)

        if H is not None:  # does the same as getTransformedCorners
            corners = FLOAT([[0, 0], [w1, 0], [w1, h1], [0, h1]])
            self.corners = np.int32(cv2.perspectiveTransform(
        ...
        defColor = (0, 255, 0)
        self.goodcolor = colors.get(
            'green', defColor)  # color for good matches
        self.badcolor = colors.get('red', defColor)  # color for bad matches
        self.kpcolor = colors.get('orange', defColor)  # color for key-points
        # color of the proyected corners
        self.framecolor = colors.get('blue', defColor)

        super(MatchExplorer, self).__init__(win, vis)
        self.cmdlist.extend(
            ["showgoods", "showbads", "framecolor", "kpcolor", "badcolor", "goodcolor"])
        self.drawline()
    ...
    def drawline(self):
        ...
        self.thick = int(
            sigmoid(vis.shape[0] * vis.shape[1], 1723567, 8080000, 5, 1))
        if self.H is not None:  # enclosing object
            rcorners = np.array([self.real2render(corner[0], corner[1])
                                 for corner in self.corners], np.int32)
            # draw rendered TM encasing
            cv2.polylines(vis, [rcorners], True, self.framecolor)
        ...
        for i, ((x1, y1), (x2, y2), inlier) in enumerate(zip(self.p1, self.p2, self.status)):  # draw points
            ...
            if self.showgoods and inlier:  # drawing circles (good keypoints)
                ...
                cv2.circle(vis, (rx1, ry1), r, gcol, -1)
                # for right keypoint (imgf)
                cv2.circle(vis, (rx2, ry2), r, gcol, -1)
            # drawing x marks (wrong keypoints)
            if self.showbads and not inlier:
                thickness = r + 5
                # for left keypoint (imgf)
                cv2.line(vis, (rx1 - r, ry1 - r),
                         (rx1 + r, ry1 + r), bcol, thickness)
                cv2.line(vis, (rx1 - r, ry1 + r),
                         (rx1 + r, ry1 - r), bcol, thickness)
                # for right keypoint (imgf)
                cv2.line(vis, (rx2 - r, ry2 - r),
                         (rx2 + r, ry2 + r), bcol, thickness)
                cv2.line(vis, (rx2 - r, ry2 + r),
                         (rx2 + r, ry2 - r), bcol, thickness)
            # drawing lines for non-onmouse event
        # detach state of the visualization for onmouse event, this only
        # contains points
        self.vis0 = vis.copy()
    ...
    def drawrelation(self):
        ...
        if self.flags & cv2.EVENT_FLAG_LBUTTON:
            ...
            for i in idxs:  # for all keypints near pointer
                # my keypoint
                (rx1, ry1), (rx2, ry2) = self.rp1[i], self.rp2[i]
                col = (self.badcolor, self.goodcolor)[
                    int(self.status[i])]  # choosing False=red,True=green
                cv2.line(cur_vis, (rx1, ry1), (rx2, ry2),
                         col, self.thick)  # drawing line
            ...
            cur_vis = cv2.drawKeypoints(
                cur_vis, kp1s, flags=4, color=self.kpcolor)
            cur_vis = cv2.drawKeypoints(
                cur_vis, kp2s, flags=4, color=self.kpcolor)
            self.rimg = cur_vis
        else:
            self.rimg = self.vis
...
class Edger(Plotim):
    ...
    def __init__(self, img,
        ...
        self.initname = "edge_"
        self.cmdlist.extend(
            ["isSIZE", "isEQUA", "isBFILTER", "isCLAHE", "showgray"])
        # best guess: (50,100,10), opencv: (9,75,75), d=-1 is filter distance
        # until sigma
    ...
    def save(self, strname=None, ext=".png", name="img"):
        if not strname:
            temp = ("E" + str(self.th1) + "_" + str(self.th2) + "_" +
                str(self.apertureSize) + "_" + str(self.L2gradient) + "_")
            name = (self.pathdata[:2] + self.initname +
                self.savename + [temp] + self.pathdata[2:])
            strname = "".join(name)
    ...
    def computeAll(self):
        ...
        if self.isSIZE:
            ...
            self.savename.append(
                "SIZE" + str(size[0]) + "_" + str(size[1]) + "_")
        else:
            img = self.data2.copy()
        ...
        if self.isCLAHE:
            ...
            self.savename.append("CLAHE" + str(clipLimit) + "_" +
                                 str(tileGridSize[0]) + "_" + str(tileGridSize[1]) + "_")
        if self.isBFILTER:
            d, sigmaColor, sigmaSpace = self.d, self.sigmaColor, self.sigmaSpace
            gray = bilateralFilter(gray, d, sigmaColor, sigmaSpace)
            self.savename.append("FILTER" + str(d) + "_" +
                                 str(sigmaColor) + "_" + str(sigmaSpace) + "_")
        self.bgr = img
        self.gray = gray
        if self.showgray:
    ...
    def th1(self, value):
        if bool(self._th1 != value):  # on change
            ...
            if self.isActiveWindow():
                cv2.createTrackbar('th1', self.win, value,
                                   self.maxth, self.onTrackbar1)
                self.computeEdge()
    ...
    def th2(self, value):
        if bool(self._th2 != value):  # on change
            self._th2 = value
            if self.isActiveWindow():
                cv2.createTrackbar('th2', self.win, value,
                                   self.maxth, self.onTrackbar2)
                self.computeEdge()
        ...
        if bool(self._maxth != value):  # on change
            ...
            if self.isActiveWindow():
                if self.th1 > value:
                    ...
                    cv2.createTrackbar(
                        'th1', self.win, self.th1, self._maxth, self.onTrackbar1)
                if self.th2 > value:
                    self.th2 = value
                else:
                    cv2.createTrackbar(
                        'th2', self.win, self.th2, self._maxth, self.onTrackbar2)
                self.computeEdge()
    ...
    def computeEdge(self):
        edge = cv2.Canny(self.gray, self.th1, self.th2,
    ...
    def windowfunc(self):
        cv2.namedWindow(self.win, self.wintype)  # create window
        cv2.createTrackbar('th1', self.win, self.th1,
                           self.maxth, self.onTrackbar1)
        cv2.createTrackbar('th2', self.win, self.th2,
                           self.maxth, self.onTrackbar2)
        cv2.resizeWindow(self.win, self.rW, self.rH)
        self.computeEdge()
...
class Imtester(Plotim):
    ...
    def __init__(self, img, win="Imtester plot"):
        ...
        if len(sz) == 2:
            ...
            self.channels = ["channel b",
                             "channel g", "channel r", "channel c"]
        self.show()
        ...
        if adaptativetoggle:
            if threshtoggle:  # Mean img,maximum,cv2.ADAPTIVE_THRESH_MEAN_C,type,blocksz,C
                thresh = cv2.adaptiveThreshold(
                    img, 1, cv2.ADAPTIVE_THRESH_MEAN_C, type, blocksz, c)
                title += ". Mean" + ti + ": "
            else:  # Gauss img,maximum,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,type,blocksz,C
                thresh = cv2.adaptiveThreshold(
                    img, 1, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, type, blocksz, c)
                title += ". Gauss" + ti + ": "
            info += ("blocksz" + i + "= " +
                str(blocksz) + ", c" + i + "= " + str(c))
        else:
    ...
    def updatevisualization(self, image, channel, th=None, items=None, thresh1=None, thresh2=None):
        ...
        if self.showhist and self.portablehist:
            ...
            if items is None:
                items = [[cv2.resize(hst, sz)]]
            else:
                items.insert(0, [cv2.resize(hst, sz)])
        if items is None:
            items = [
                ["zoom(x{self.rxZoom}({self.rx1}-{self.rx2}), y{self.ryZoom}({self.ry1}-{self.ry2})) "]]
        ...
        if thresh1 is not None and thresh2 is not None:
            # MAKE VISUALIZATION
            self.data = padVH(
                [data, [thresh1 * self.maximum, thresh2 * self.maximum]], 100, 100)[0]
        else:
    ...
    def builtcmd(self):
        ...
        for i in cmdlist:
            if i not in self.cmdlist:
                self.cmdlist.append(i)
        # self.doubleth # "self._timeitems"
        binary = ["if self.doubleth: self.type2=self.binary",
                  "if not self.doubleth: self.type1=self.binary"]
        binary_inv = ["if self.doubleth: self.type2=self.binary_inv",
                      "if not self.doubleth: self.type1=self.binary_inv"]
        tozero = ["if self.doubleth: self.type2=self.tozero",
                  "if not self.doubleth: self.type1=self.tozero"]
        tozero_inv = ["if self.doubleth: self.type2=self.tozero_inv",
                      "if not self.doubleth: self.type1=self.tozero_inv"]
        truncate = ["if self.doubleth: self.type2=self.trunc",
                    "if not self.doubleth: self.type1=self.trunc"]
        ...
        if len(sz) == 2:
            ...
            for i in range(sz[2]):
                self.cmdeval[self.channels[i]
                             ] = "self.channel=self.data0[:,:," + str(i) + "].copy()"

        self.cmdlist.extend(list(self.cmdeval.keys()))

    def computefunc(self, image=None):
        # this function is designed to work with countless initializations of
        # image after self.builtcmd()
        if image is None:
            image = self.data0
        ...
        if self.sigmoide:
            info += (". Sigmoide: alfa= " +
                str(self.alfa) + ", beta= " + str(self.beta))
            chimg = sigmoid(chimg.astype("float"), self.alfa,
                            self.beta).astype("uint8")
        ...
        if flags == 0 or type(ksize) is np.ndarray:  # convolution
            ...
            chimg = cv2.bilateralFilter(
                chimg, self.d, self.sigmaColor, self.sigmaSpace)
            info += ". Bilateral F: d= " + str(self.d) + ", sigmaColor= " + str(
                self.sigmaColor) + ", sigmaSpace= " + str(self.sigmaSpace)
        else:
            info += ". No Filter"
        ...
        if self.doubleth:
            thresh1, info1, title1 = self.applythresh(
                chimg, self.type1, self.adaptoggle1, self.thtoggle1, self.th1, self.blocksz1, self.c1, i="1", ti="1")
            thresh2, info2, title2 = self.applythresh(
                chimg, self.type2, self.adaptoggle2, self.thtoggle2, self.th2, self.blocksz2, self.c2, i="2", ti="2")
            if self.operation:
            ...
            self.th, info1, title1 = self.applythresh(
                chimg, self.type1, self.adaptoggle1, self.thtoggle1, self.th1, self.blocksz1, self.c1, i="1", ti="1")
            # DETECT TYPE
