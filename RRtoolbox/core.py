...
def tools(instance, modules):
    for key in list(modules.keys()):
        ...
        classmethods = dict(inspect.getmembers(
            moduleTool, predicate=inspect.ismethod))
        if "__init__" in list(classmethods.keys()):
            del classmethods["__init__"]
        for method in classmethods:
            # convert to bound method
            fn = types.MethodType(
                classmethods[method], instance, instance.__class__)
