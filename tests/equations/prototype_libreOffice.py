...
modifier_dict = {
    ...
    'checkLoaded': lambda s: r' checkLoaded{'+s+r'}',
    'breve': lambda s: r' breve{'+s+r'}',
    'acute': lambda s: r' acute{'+s+r'}',
    'grave': lambda s: r' grave{'+s+r'}',
    'tilde': lambda s: r' tilde{'+s+r'}',
...
class libreOfficePrinter(Printer):
    ...
    def __init__(self, settings=None):
        ...
        self._delim_dict = {"(":")","[":"]","ldbracket":"rdbracket","lbrace":"rbrace","langle":"rangle",
                            "lceil":"rceil","lfloor":"rfloor","lline":"rline","ldline":"rdline"}
            ...
            if self._settings['fold_short_frac'] \
                ...
                if self._needs_mul_brackets(numer, last=True):
                    ...
                    if self._needs_mul_brackets(b, last=True):
                        tex += r" {%s} over {%s}%s left(%s right)" \
                            % (convert(a), sdenom, separator, convert(b))
                    else:
                        tex += r" {%s} over {%s}%s%s" \
                            % (convert(a), sdenom, separator, convert(b))
                else:
                    tex += r" {1} over {%s}%s%s" % (sdenom, separator, snumer)
            else:
                tex += r" {%s} over {%s}" % (snumer, sdenom)
...
def translate(s):
    # Process the rest
    if s in greek_letters_set or s.lower() in greek_letters_set:
        return "%" + s
    else:
        return s
...
if __name__ == "__main__":
    ...
    from sympy import latex, symbols, Matrix, MatrixSymbol, Eq, S, collect
    ...
    _sATsB = Matrix(sATsB)
    _sBToB = Matrix([[sin(Bow/Bsw),0,0],[0,Boh/Bsh,0],[0,0,1]])
    print(libreOffice(Eq(oAToB, sBToB*sATsB*oATsA)))
    print(libreOffice(Eq(oAToB, _sBToB*sATsB*_oATsA)))
    print(libreOffice(_sBToB*_sATsB*_oATsA))